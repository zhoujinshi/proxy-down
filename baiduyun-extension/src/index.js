...
import api from 'proxy-down-extension-sdk'
...
util.onBdyInit(async () => {
  ...
  $(document).on('click', 'a[data-menu-id=pd-batch]', async function() {
    ...
    downHandle('batch', downFiles, async result => {
      ...
      /*
      压缩链接下载的文件大小和文件名需要通过proxy Down解析API解析出来，百度云的api里没有返回对应信息
      压缩链接不需要传递cookie
      */
      try {
  ...
  //直接推送下载，把选中的文件全部解析成直链，推送到proxy Down下载，不弹下载框使用默认下载路径和连接数
